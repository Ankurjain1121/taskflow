...
/// Internal helper: get webhook's project_id
async fn get_webhook_project_id_internal(
    pool: &PgPool,
    webhook_id: Uuid,
...
pub async fn list_webhooks(
    ...
    if !verify_project_membership_internal(pool, project_id, user_id).await? {
        return Err(WebhookQueryError::NotProjectMember);
    }
...
/// Update a webhook.
/// Verifies project membership via the webhook's project_id.
pub async fn update_webhook(
    ...
    let project_id = get_webhook_project_id_internal(pool, webhook_id).await?;
...
/// Get all active webhooks for a project that match a given event type.
...
mod tests {
    ...
    async fn setup_full(pool: &PgPool) -> (Uuid, Uuid, Uuid, Uuid, Uuid) {
        ...
        let bwc =
            projects::create_project(pool, "WH Test Project", None, ws_id, tenant_id, user_id)
                .await
    ...
    async fn test_create_webhook() {
        ...
        let webhook = create_webhook(&pool, project_id, input, user_id, tenant_id)
            .await
            .expect("create_webhook");

        assert_eq!(webhook.project_id, project_id);
    ...
    async fn test_list_webhooks() {
        ...
        create_webhook(&pool, project_id, input1, user_id, tenant_id)
            .await
        ...
        create_webhook(&pool, project_id, input2, user_id, tenant_id)
            .await
            .expect("create webhook 2");

        let webhooks = list_webhooks(&pool, project_id, user_id)
            .await
    ...
    async fn test_get_active_webhooks_for_event() {
        ...
        create_webhook(&pool, project_id, input, user_id, tenant_id)
            .await
            .expect("create webhook for event test");

        let active = get_active_webhooks_for_event(&pool, project_id, "task.created")
            .await
        ...
        let no_match = get_active_webhooks_for_event(&pool, project_id, "nonexistent.event")
            .await
    ...
    async fn test_webhook_not_board_member() {
        ...
        let result = create_webhook(&pool, project_id, input, other_user.id, tenant_id).await;
        assert!(
            result.is_err(),
