...
pub async fn create_task_from_template(
    ...
    sqlx::query(
        r#"
        INSERT INTO tasks (
            id, title, description, priority, project_id, column_id,
...
mod tests {
    ...
    async fn setup_user(pool: &PgPool) -> (Uuid, Uuid) {
        let user = auth::create_user_with_tenant(pool, &unique_email(), "TaskTmpl User", FAKE_HASH)
            .await
    ...
    async fn setup_full(pool: &PgPool) -> (Uuid, Uuid, Uuid, Uuid, Uuid) {
        ...
        let bwc =
            projects::create_project(pool, "TaskTmpl Project", None, ws_id, tenant_id, user_id)
                .await
