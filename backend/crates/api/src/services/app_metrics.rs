...
pub fn record_email_queued(event_type: &str) {
    counter!("taskbolt_emails_queued_total", "event_type" => event_type.to_string()).increment(1);
}

/// Record an email send attempt (success or failure).
pub fn record_email_sent(event_type: &str, status: &str) {
    counter!("taskbolt_emails_sent_total", "event_type" => event_type.to_string(), "status" => status.to_string()).increment(1);
}

/// Record a report generation event.
pub fn record_report_generated(report_type: &str, format: &str) {
    counter!("taskbolt_reports_generated_total", "type" => report_type.to_string(), "format" => format.to_string()).increment(1);
}

/// Record an HTTP request with method, path, status code, and duration.
pub fn record_http_request(method: &str, path: &str, status: u16, duration_secs: f64) {
    counter!("taskbolt_http_requests_total", "method" => method.to_string(), "path" => path.to_string(), "status" => status.to_string()).increment(1);
    histogram!("taskbolt_http_request_duration_seconds", "method" => method.to_string(), "path" => path.to_string()).record(duration_secs);
}

/// Set the current email queue depth gauge.
pub fn set_email_queue_depth(depth: f64) {
    gauge!("taskbolt_email_queue_depth").set(depth);
}

/// Set the current active WebSocket connections gauge.
pub fn set_active_websocket_connections(count: f64) {
    gauge!("taskbolt_active_websocket_connections").set(count);
}
