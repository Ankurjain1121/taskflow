...
describe('TaskService', () => {
  ...
  describe('getTaskDetails()', () => {
    it('should GET /api/tasks/:taskId/details', () => {
      service.getTaskDetails('task-1').subscribe();

      const req = httpMock.expectOne('/api/tasks/task-1/details');
      expect(req.request.method).toBe('GET');
      req.flush({});
    });
  });
  ...
  describe('listByProject()', () => {
    it('should GET /api/projects/:projectId/tasks with no filters', () => {
      service.listByProject('proj-1').subscribe();

      const req = httpMock.expectOne('/api/projects/proj-1/tasks');
      expect(req.request.method).toBe('GET');
      req.flush([]);
    });
    ...
    it('should append filter params when provided', () => {
      service
        .listByProject('proj-1', { priority: 'high', assignee_id: 'user-1' })
        .subscribe();

      const req = httpMock.expectOne(
        (r) => r.url === '/api/projects/proj-1/tasks',
      );
      expect(req.request.method).toBe('GET');
      expect(req.request.params.get('priority')).toBe('high');
      expect(req.request.params.get('assignee_id')).toBe('user-1');
      req.flush([]);
    });

    it('should skip null/empty filter values', () => {
      service
        .listByProject('proj-1', { priority: '', assignee_id: undefined })
        .subscribe();
  ...
  describe('moveTaskPosition()', () => {
    it('should PATCH /api/tasks/:taskId/move', () => {
      const moveReq = { target_column_id: 'col-2' };

      service.moveTaskPosition('task-1', moveReq).subscribe();
  ...
  describe('createProjectTask()', () => {
    it('should POST /api/projects/:projectId/tasks', () => {
      const createReq = {
        title: 'New Task',
        priority: 'high',
        column_id: 'col-1',
      ...
      service.createProjectTask('proj-1', createReq).subscribe();
  ...
  describe('listSubtasks()', () => {
    it('should GET /api/tasks/:parentTaskId/subtasks', () => {
      service.listSubtasks('task-1').subscribe();
  ...
  describe('createSubtask()', () => {
    it('should POST /api/tasks/:parentTaskId/subtasks', () => {
      const subtaskReq = { title: 'Sub-task', priority: 'low' };

      service.createSubtask('task-1', subtaskReq).subscribe();

      const req = httpMock.expectOne('/api/tasks/task-1/subtasks');
      expect(req.request.method).toBe('POST');
      expect(req.request.body).toEqual(subtaskReq);
      req.flush(MOCK_TASK);
    });
  });
