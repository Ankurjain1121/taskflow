  ...
  template: `
    <div>
      <div class="flex items-center justify-between mb-6">
        <div>
          <h2 class="text-xl font-bold text-[var(--foreground)]">Task Templates</h2>
          <p class="mt-1 text-sm text-[var(--muted-foreground)]">
            Manage reusable task templates. Use them when creating new tasks to save time.
          </p>
        </div>
        <button
          pButton
      ...
      @if (loading()) {
        ...
        <div class="text-center py-12 bg-[var(--card)] rounded-lg border border-[var(--border)]">
          <i class="pi pi-copy text-4xl text-[var(--muted-foreground)] mb-3"></i>
          <h3 class="text-base font-medium text-[var(--foreground)] mb-1">No templates yet</h3>
        ...
        <div class="space-y-3">
          @for (template of templates(); track template.id) {
            <div class="bg-[var(--card)] border border-[var(--border)] rounded-lg p-4 flex items-start gap-4 hover:shadow-sm transition-shadow">
              <div class="flex-1 min-w-0">
                @if (editingId() === template.id) {
                  ...
                  <div class="flex items-center gap-2 mb-1">
                    <span class="font-medium text-[var(--foreground)]">{{ template.name }}</span>
                    <p-tag
                  ...
                  <p class="text-sm text-[var(--muted-foreground)] mb-1">{{ template.task_title }}</p>
                  @if (template.description) {
                    <p class="text-xs text-[var(--muted-foreground)]">{{ template.description }}</p>
                  }
    ...
    >
      <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-1">
          ...
          <input pInputText [(ngModel)]="newName" placeholder="e.g. Bug Report Template" class="w-full" />
        </div>
        <div class="flex flex-col gap-1">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-sm font-medium text-[var(--foreground)]">Task Title</label>
          <input pInputText [(ngModel)]="newTaskTitle" placeholder="Default task title" class="w-full" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-sm font-medium text-[var(--foreground)]">Task Description</label>
          <textarea pTextarea [(ngModel)]="newTaskDescription" rows="3" placeholder="Default task description" class="w-full"></textarea>
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-sm font-medium text-[var(--foreground)]">Priority</label>
          <p-select
