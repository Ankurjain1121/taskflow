  ...
  template: `
    <div class="widget-card p-5 h-full">
      ...
      @if (data().length === 0) {
        ...
        >
          <p class="text-sm">No burnup data available</p>
        </div>
      } @else {
        <p-chart
          type="line"
...
export class BurnupChartComponent {
  data = input<BurnupPoint[]>([]);

  latestCompleted = computed(() => {
    const d = this.data();
    if (d.length === 0) return 0;
    return d[d.length - 1].completed;
  });
  ...
  chartData = computed(() => {
    ...
    return {
      ...
      datasets: [
        {
          label: 'Total Scope',
          data: points.map((p) => p.total_scope),
          fill: false,
