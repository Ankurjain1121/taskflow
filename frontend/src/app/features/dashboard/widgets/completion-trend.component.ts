        ...
        <div class="flex gap-0.5 rounded-lg p-0.5" style="background: var(--muted)">
          @for (option of dayOptions; track option) {
            <button
              ...
              [style.background]="selectedDays() === option ? 'var(--card)' : 'transparent'"
              [style.color]="selectedDays() === option ? 'var(--primary)' : 'var(--muted-foreground)'"
              [style.box-shadow]="selectedDays() === option ? '0 1px 2px rgba(0,0,0,0.05)' : 'none'"
            >
              {{ option }}d
            </button>
          }
      ...
      @if (loading()) {
        ...
        <div class="mt-3 flex items-center justify-center gap-4 text-sm">
          <div class="flex items-center gap-2">
            <div class="w-2.5 h-2.5 bg-indigo-500 rounded"></div>
            <span style="color: var(--muted-foreground)">Tasks Completed</span>
          </div>
          <span style="color: var(--muted-foreground)">&#183;</span>
          <span class="font-semibold font-display" style="color: var(--foreground)">
