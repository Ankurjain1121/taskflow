      ...
      <div class="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="flex items-center justify-center">
              <svg viewBox="0 0 100 100" class="w-48 h-48">
                ...
                <text
                  ...
                  y="46"
                  text-anchor="middle"
                  class="text-2xl font-bold fill-[var(--card-foreground)]"
                  font-size="16"
                >
                  {{ completionPercent() }}%
                </text>
            ...
            @if (burndownPoints().length > 0) {
              ...
              >
                ...
                @for (tick of burndownYTicks(); track tick) {
                  <line
                    [attr.x1]="40"
                    [attr.y1]="burndownYScale()(tick)"
                    [attr.x2]="chartWidth - 10"
                    [attr.y2]="burndownYScale()(tick)"
                    stroke="#e5e7eb"
                    stroke-width="0.5"
                  />
                  <text
                    [attr.x]="36"
                    [attr.y]="burndownYScale()(tick) + 3"
                    text-anchor="end"
                    fill="#9ca3af"
                    font-size="8"
                  >
          ...
          >
            ...
            <div class="space-y-3">
              @for (
                item of report()!.priority_distribution;
                track item.priority
              ) {
                <div class="flex items-center gap-3">
                  <span
                    class="text-xs text-[var(--muted-foreground)] w-14 text-right capitalize"
                    >{{ item.priority }}</span
                  >
                  <div
                    class="flex-1 bg-[var(--secondary)] rounded-full h-6 relative overflow-hidden"
                  >
                    <div
                  ...
                  <span
                    class="text-xs font-medium text-[var(--foreground)] w-8"
                    >{{ item.count }}</span
                  >
                </div>
              }
                ...
                @for (a of report()!.assignee_workload; track a.user_id) {
                  <div class="flex items-center gap-3">
                    <div class="flex items-center gap-2 w-28 min-w-0">
                      <div
                        class="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-xs font-medium text-primary flex-shrink-0"
                      >
                        {{ a.name.charAt(0) }}
                      </div>
                      <span
                    ...
                    <span
                      class="text-xs text-[var(--muted-foreground)] w-12 text-right"
                      >{{ a.completed_tasks }}/{{ a.total_tasks }}</span
                    >
                  </div>
                }
              </div>
          ...
          >
            ...
            @if (hasOverdue()) {
              <div class="flex items-end justify-center gap-8 h-40">
                @for (
                  bucket of report()!.overdue_analysis;
                  track bucket.bucket
                ) {
                  <div class="flex flex-col items-center gap-2">
                    ...
                    <div
                      ...
                      [style.opacity]="
                        0.5 + (bucket.count / maxOverdue()) * 0.5
                      "
                    ></div>
                    <span
                      class="text-xs text-[var(--muted-foreground)] text-center"
                      >{{ bucket.bucket }}</span
                    >
                  </div>
                }
...
export class ReportsViewComponent implements OnInit {
  ...
  completionPercent = computed(() => {
    ...
    return Math.round(
      (r.completion_rate.completed / r.completion_rate.total) * 100,
    );
  });
  ...
  pieSegments = computed(() => {
    ...
    return createPieSegments([
      {
        value: r.completion_rate.completed,
        color: '#22c55e',
        label: 'Completed',
      },
      {
        value: r.completion_rate.remaining,
        color: '#e5e7eb',
        label: 'Remaining',
      },
    ]);
  ...
  burndownYScale = computed(() => {
    const r = this.report();
    if (!r || r.burndown.length === 0)
      return scaleLinear(
        [0, 1],
        [this.chartHeight - this.chartPadding.bottom, this.chartPadding.top],
      );
  ...
  burndownArea = computed(() => {
    return createAreaPath(
      this.burndownPoints(),
      this.chartHeight - this.chartPadding.bottom,
    );
  });
