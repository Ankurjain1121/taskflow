        ...
        <p-card class="mb-6">
          <div class="p-4">
            ...
            <div class="space-y-4">
              ...
              <div class="flex flex-col gap-2 w-full">
                <label for="webhookEvents" class="text-sm font-medium"
                  >Events</label
                >
                <p-multiSelect
                  id="webhookEvents"
                ...
                <div class="space-y-3 pt-2">
                  ...
                  <div class="flex items-center gap-3 border-t pt-3">
                    ...
                    <button
                      ...
                      label="Deliveries"
                      icon="pi pi-history"
                      (click)="loadDeliveries(webhook.id)"
                    ></button>
                    <button
                      pButton
                      [text]="true"
                      ...
                      @if (deliveriesMap()[webhook.id]!.length === 0) {
                        ...
                        <div class="space-y-1">
                          @for (d of deliveriesMap()[webhook.id]!; track d.id) {
                            <div
                              class="flex items-center gap-2 text-xs py-1 border-b last:border-0"
                            >
                              <span
                                class="w-2 h-2 rounded-full"
...
export class WebhookSettingsComponent implements OnInit {
  ...
  toggleWebhook(webhook: Webhook, isActive: boolean) {
    this.webhookService
      .updateWebhook(webhook.id, { is_active: isActive })
      .subscribe({
        next: (updated) => {
          this.webhooks.update((w) =>
            w.map((wh) => (wh.id === updated.id ? updated : wh)),
          );
        },
      });
  }
