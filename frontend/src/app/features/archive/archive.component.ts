...
import { ArchiveService, ArchiveItem } from '../../core/services/archive.service';
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
      <header class="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Archive</h1>
          <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Deleted items are kept for 30 days before permanent removal</p>
        </div>
      </header>
      ...
      <main class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if (loading()) {
              ...
              <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1 space-y-2">
                    ...
                    <div class="skeleton skeleton-text w-32" style="height: 0.625rem"></div>
                  </div>
                </div>
              </div>
            }
          ...
          <div class="text-center py-16">
            ...
            <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">Archive is empty</h3>
              ...
              <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-gray-500 dark:text-gray-400">
                    {{ item.entity_type | titlecase }} &middot;
                    Deleted {{ formatDate(item.deleted_at) }} &middot;
                    <span [class]="item.days_remaining <= 7 ? 'text-red-500' : ''">
                ...
                <div class="flex items-center gap-2">
                  <button (click)="restore(item)"
                          ...
                          class="text-xs font-medium px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors disabled:opacity-50">
                  ...
                  @if (isAdmin()) {
                    <button (click)="permanentlyDelete(item)"
          ...
          @if (nextCursor()) {
            <div class="mt-6 text-center">
              <button (click)="loadMore()"
                      ...
                      class="px-4 py-2 text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 disabled:opacity-50">
                @if (loadingMore()) {
                  Loading...
                } @else {
                  Load more
...
export class ArchiveComponent implements OnInit {
  ...
  restore(item: ArchiveItem): void {
    ...
    this.archiveService.restore(item.entity_type, item.entity_id).subscribe({
      next: () => {
        this.items.set(this.items().filter((i) => i.entity_id !== item.entity_id));
  ...
  permanentlyDelete(item: ArchiveItem): void {
    ...
    this.archiveService.permanentlyDelete(item.entity_type, item.entity_id).subscribe({
      next: () => {
        this.items.set(this.items().filter((i) => i.entity_id !== item.entity_id));
