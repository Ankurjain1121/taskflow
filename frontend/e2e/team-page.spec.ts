...
test.describe('Team Page', () => {
  ...
  test('workspace section shows workspace name', async ({ page }) => {
    ...
    await expect(page.locator('h2:has-text("Team Page WS")')).toBeVisible({ timeout: 15000 });
  });
  ...
  test('member card shows current user', async ({ page }) => {
    ...
    await expect(page.locator(`text=${TEST_NAME}`)).toBeVisible({ timeout: 15000 });
  });
  ...
  test('stats section shows Active, Overdue, Done counts', async ({ page }) => {
    await page.goto('/team');
    await page.waitForLoadState('networkidle');
    ...
    await expect(page.locator('text=Overdue').first()).toBeVisible({ timeout: 15000 });
  ...
  test('"View details" navigates to workspace team page', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/workspace\/[a-f0-9-]+\/team/, { timeout: 15000 });
  });
  ...
  test('loading state resolves to content', async ({ page }) => {
    ...
    const hasWorkspaces = await page.locator('h2').first().isVisible().catch(() => false);
    const hasEmptyState = await page.locator('text=No team data').isVisible().catch(() => false);
  ...
  test('page is accessible from sidebar navigation', async ({ page }) => {
    ...
    const teamLink = page.locator('a[routerLink="/team"], a[href="/team"]').first();
